...
sidebar: sidebar 
keywords:  
summary: 'Le serveur d"API Hello est un exemple de programme qui explique comment appeler une API REST dans Active IQ Unified Manager à l"aide d"un client REST simple. L"exemple de programme vous fournit des détails de base sur le serveur API au format JSON \(le serveur ne prend en charge que le format application/json\).' 
---
= Bonjour serveur API
:allow-uri-read: 
:icons: font
